...
为使用 OpenSpec 进行规格驱动开发的 AI 编码助手提供的指南。
...
2. 选择唯一的动词开头的 `change-id`，并在 `openspec/changes/<id>/` 下搭建 `proposal.md`、`tasks.md`、可选的 `design.md` 和规格增量。
...
6. **更新检查清单** - 在所有工作完成后，将每个任务设置为 `- [x]`，使列表反映实际情况
...
- 如果能力发生变化则更新 `specs/`
- 对于仅工具相关的变更使用 `openspec archive <change-id> --skip-specs --yes`（始终明确传递 change ID）
...
- 如果请求不明确，在搭建脚手架前先提出 1-2 个澄清问题

### 搜索指导

- 枚举规格：`openspec spec list --long`（或使用 `--json` 用于脚本）
...
**迁移**：[如何处理]
```
如果多个能力受到影响，在 `changes/[change-id]/specs/<capability>/spec.md` 下为每个能力创建多个增量文件。
...
- 使用"必须"表示规范性需求（除非有意使用非规范性，否则避免使用"应该"/"可以"）
...
- ADDED：引入可独立作为需求的新能力或子能力。当变更是正交的（例如，添加"Slash 命令配置"）而不是改变现有需求的语义时，优先使用 ADDED。
- MODIFIED：更改现有需求的行为、范围或验收标准。始终粘贴完整的更新后需求内容（标题 + 所有场景）。归档器将用您在此处提供的内容替换整个需求；部分增量将丢失先前的细节。
- RENAMED：仅在名称更改时使用。如果您也更改行为，请使用 RENAMED（名称）加上引用新名称的 MODIFIED（内容）。

常见陷阱：使用 MODIFIED 添加新关注点而不包含先前的文本。这会在归档时导致细节丢失。如果您没有明确更改现有需求，请改为在 ADDED 下添加新需求。

正确编写 MODIFIED 需求：
1) 在 `openspec/specs/<capability>/spec.md` 中定位现有需求。
2) 复制整个需求块（从 `### 要求：...` 到其场景）。
3) 将其粘贴到 `## MODIFIED Requirements` 下并编辑以反映新行为。
4) 确保标题文本完全匹配（不区分空格），并保留至少一个 `#### 场景：`。
...
- 优先使用动词主导的前缀：`add-`、`update-`、`remove-`、`refactor-`
