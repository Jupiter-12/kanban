...
- 对话开始时查询 `回忆` 参数 `project_path` 为 git 的根目录，可以通过git rev-parse --show-toplevel获得
...
当调试的项目是前端项目时，可以使用`chrome-devtools`中的工具进行验证与调试但是你开始调试前需要用`cunzhi`工具询问。
